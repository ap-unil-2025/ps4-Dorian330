...
def merge_dictionaries(dict1, dict2):
    ...
    merged = dict1.copy()  # on copie le premier dictionnaire
    merged.update(dict2)   # on ajoute le deuxième (écrase les doublons)
    return merged
...
def count_word_frequency(text):
    ...
    for char in string.punctuation:  # on enlève la ponctuation
        text = text.replace(char, "")
    words = text.split()  # on sépare en mots
    freq = {}
    for word in words:
        if word in freq:
            freq[word] += 1
...
def group_by_first_letter(words):
    ...
    result = {}
    for word in words:
        first_letter = word[0]  # première lettre
        if first_letter not in result:
            result[first_letter] = []  # crée la liste si la clé n'existe pas
        result[first_letter].append(word)  # ajoute le mot à la liste
    return result
...
def calculate_grades_average(students):
    ...
    for student, grades in students.items():
        avg = sum(grades) / len(grades)  # moyenne
        averages[student] = round(avg, 2)  # arrondie à 2 décimales
    return averages
...
def nested_dict_access(data, keys):
    ...
    for key in keys:
        if key in current:
            current = current[key]  # descend d’un niveau
        else:
            return None  # clé manquante → retourne None
    return current
